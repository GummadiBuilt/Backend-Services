...
public interface TenderApplicantsDao extends JpaRepository<TenderApplicants, Integer> {
    ...
    @Query(value = "select ta.applicant_rank, ta.id, ta.application_status, ta.tender_info_id , ta.applicant_form_id, " +
            ...
            "(select SUM((value ->>'revenue')\\:\\:float) from json_array_elements(coalesce(af.turn_over_details\\:\\:json,'[{}]'\\:\\:json))) as total_revenue,"+
            "(select SUM((value ->>'Total Price')\\:\\:float) from json_array_elements(coalesce(tbi.tender_finance_info\\:\\:json,'[{}]'\\:\\:json))) as financial_info,"+
            ...
            "left join application_form af on af.application_user_id = au.id and af.tender_info_id  = ti.id " +
