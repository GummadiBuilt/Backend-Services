...
public interface TenderApplicantsDao extends JpaRepository<TenderApplicants, Integer> {
    ...
    @Query(value = "select ta.applicant_rank, ta.id, ta.application_status, ta.tender_info_id , ta.applicant_form_id, " +
            ...
            "ti.workflow_step, " +
            "case when ti.workflow_step = 'IN_REVIEW' then COALESCE (tbi.tender_document_name,'') else '' end as tender_document," +
            "case when ti.workflow_step = 'IN_REVIEW' then COALESCE (tbi.tender_document_size,0) else 0 end as tender_document_size, " +
            "case when ti.workflow_step = 'IN_REVIEW' then COALESCE (to_json(tbi.tender_finance_info\\:\\:text) #>> '{}', to_json(''\\:\\:text) #>> '{}') else to_json(''\\:\\:text) #>> '{}'  end as tender_finance_info  " +
